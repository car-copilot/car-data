...
def train_model(influxdb_data_all_trips):
    ...
    sequences = create_windows(influxdb_data_all_trips, WINDOW_SIZE, influxdb_data_all_trips.columns[:-2])
    ...
    environment_model = create_model_for_environment(features.shape[2], WINDOW_SIZE)
    ...
    driving_score_model = create_model_for_score(features.shape[2], WINDOW_SIZE)
